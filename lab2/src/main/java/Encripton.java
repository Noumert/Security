...
public class Encripton {
    ...
    public static Map<String, Integer> getCharFrequencies2(String text) {
        Map<String,Integer> r = new HashMap<>();
        for (int i = 0; i < text.length()-1; i+=4) {
            String s = "" + text.charAt(i) + text.charAt(i + 1) + text.charAt(i + 2) + text.charAt(i + 3);
            r.putIfAbsent(s,StringUtils.countMatches(text, s));
        }
        r.entrySet().stream().sorted(Comparator.comparing(Map.Entry::getValue)).forEach(System.out::println);
        return r;
    }
    ...
    public static void xorRepeatingEncoder(String text) {
        ...
        System.out.println("Real key: WVTESWPQRMGEPWRVUWPSVPVQMUL0PV");
        System.out.print("Most possible text: ");
        String decodedText = xorEncoderR(text, bestKey);
        System.out.println(decodedText);
        System.out.print("Real text: ");
        decodedText = xorEncoderR(text, "WVTDQWPQRMGEPWRVUWPSVPVQMUL0PV");
        System.out.println(decodedText);
